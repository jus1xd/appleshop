...
const Header = () => {
  ...
  const compareItemCounter = useAppSelector((state) => state.compareReducer.compareItems.length);
  const cartItemCounter = useAppSelector((state) => state.productsReducer.cart.length);
        ...
        <div className={s.container}>
          <div className={s.header_inner}>
            ...
            <div className={s.search}>
              <input
                className={findActive ? `${s.find_input} ${s.active}` : s.find_input}
                value={searchString}
                type="text"
                placeholder="Искать.."
            ...
            <nav className={s.nav}>
              {/* <Link href="/compare">
                <a className={`${s.nav_item} ${s.auth_btn}`}>
                  <div className={s.icon_container}>
                    ...
                    {compareItemCounter !== 0 ? <div className={s.item_counter}>{compareItemCounter}</div> : null}
                  </div>
                  <div>Сравнение</div>
                </a>
              </Link> */}
              <Link href="/basket">
                ...
                >
                  <div className={s.icon_container}>
                    ...
                    {cartItemCounter !== 0 ? <div className={s.item_counter}>{cartItemCounter}</div> : null}
                  </div>
                  <div>Корзина</div>
                </a>
              </Link>
              {cartItems.length !== 0 ? <BasketPopup active={basketActive} setActive={setBasketActive} /> : null}
              {authState.isAuth ? (
                <Link href="/profile">
                  <a
              ...
              <ProfilePopup active={profileActive} setActive={setProfileActive} authState={authState} />
            </nav>
          </div>
        </div>
